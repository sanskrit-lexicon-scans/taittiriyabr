# coding=utf-8
""" make_js_index.py for app0 of taittiriyabr
"""
...
""" 
 def __init__(self,line,iline):
  ...
  self.vpstr = parm_vpstr_format % (self.vol,self.page)  # external page
  ...
  self.vipstr = parm_vpstr_format %(self.vol,self.ipage)
 def todict(self):
  if self.fromvx == None:
   self.fromx = ''
  e = {
...
def remove_dups(pagerecs):
 ...
 for pagerec in pagerecs:
  if prev == None:
   ...
   print('dupa: prev=',prev.line)
   print('dupa:  cur=',pagerec.line)
   print()
   recs.append(pagerec)
   prev = pagerec
  else:
